...
from ARWgan.try_6.decoder import Decoder
from ARWgan.try_6.encoder import Encoder
...
attacks_list = [
                ...
                {'func':smoothing_mean, 'name':'Average Filter', 'params': np.array([3, 5, 7, 9]), 'active':False}, # Smoothing mean
                {'func':smoothing_gaussian, 'name':'Gaussian Filter', 'params':np.arange(1, 3, 0.2), 'active':False}, # Smoothing gaussian
                {'func':sharpening, 'name':'Sharpening', 'params':np.arange(1, 50, 1), 'active':False},
                {'func':cropping_fillzero, 'name':'Cropping ZeroFill', 'params':np.arange(0.01, 0.4, 0.05), 'active':False}, # Cropping random patches and set to zero
                {'func':gaussin_noise, 'name':'Gaussian Noise', 'params':np.arange(1, 30, 1), 'active':False}, # Gaussian noise},
                {'func':salt_pepper_noise, 'name':'Salt & Pepper Noise', 'params':np.arange(0.01, 0.2, 0.01), 'active':False}, # Salt and pepper noise}
                {'func':median_filt, 'name': 'Median Filter', 'params':np.array([3, 5, 7, 9], dtype=np.int32),'active':False},
                {'func':painting, 'name':'Painting', 'params':np.array([1, 3, 6, 9, 12, 15, 16, 18], dtype=np.int32), 'active':False},
