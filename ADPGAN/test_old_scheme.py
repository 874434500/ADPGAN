...
from ARWgan.try_10.decoder import Decoder
from ARWgan.try_10.encoder import Encoder
...
def buildModel(model_path, patch_rows=32, patch_cols=32, channels=1, block_size=8, use_circular=True):
    ...
    # Rearrange input
    rearranged_img = l1 = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': block_size}, name='rearrange_img')(
        input_img)

    dct_layer = layers.Conv2D(64, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                              name='dct1')
    dct_layer2 = layers.Conv2D(64, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                               name='dct2')
    idct_layer = layers.Conv2D(64, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                               name='idct')
    ...
    # Encoder
    encoder_model = layers.Conv2D(64, (1, 1), dilation_rate=1, activation='elu', padding='same', name='enc_conv1')(
        encoder_input)
    encoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='enc_conv2')(
        encoder_model)
    encoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='enc_conv3')(
        encoder_model)
    encoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='enc_conv4')(
        encoder_model)
    encoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='enc_conv5')(
        encoder_model)
    encoder_model = layers.Conv2D(64, (2, 2), dilation_rate=1, activation='elu', padding='same',
                                  name='enc_conv2_1')(encoder_model)
    encoder_model = layers.Conv2D(64, (3, 3), dilation_rate=1, activation='elu', padding='same',
                                  name='enc_conv2_2')(encoder_model)


    encoder_model = idct_layer(encoder_model)

    # Strength
    encoder_model = layers.Lambda(multiply_scalar, arguments={'scalar': input_strenght_alpha}, name='strenght_factor')(
        encoder_model)
    ...
    decoder_model = layers.Conv2D(64, (1, 1), dilation_rate=1, activation='elu', padding='same', name='dec_conv1')(
        decoder_model)
    decoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='dec_conv2')(
        decoder_model)
    decoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='dec_conv3')(
        decoder_model)
    decoder_model = conv2d_layer(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='dec_conv4')(
        decoder_model)

    decoder_model = layers.Conv2D(64, (2, 2), dilation_rate=1, activation='elu', padding='same', name='dec_con2_1')(
        decoder_model)
    decoder_model = layers.Conv2D(64, (3, 3), dilation_rate=1, activation='elu', padding='same', name='dec_con2_2')(
        decoder_model)

    decoder_model = layers.Conv2D(64 * 2, (1, 1), dilation_rate=1, activation='elu', padding='same',
                                  name='dec_output_depth2space_4')(decoder_model)
    decoder_model = layers.Conv2D(64, (1, 1), dilation_rate=1, activation='elu', padding='same',
                                  name='dec_output_depth2space_3')(decoder_model)
    decoder_model = layers.Conv2D(64 // 2, (1, 1), dilation_rate=1, activation='elu', padding='same',
                                  name='dec_output_depth2space_2')(decoder_model)
...
attacks_list = [
    {'func': jpg, 'name': 'JPEG', 'params': np.array([90, 70, 50, 30, 20]), 'active': True},  # JPEG
