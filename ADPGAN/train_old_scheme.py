...
def UniformNoise(x, val):
    noise = tf.random.uniform(shape=[128,4,4,64],minval=-val,maxval=val,dtype=tf.float32,seed=None)
    return x + noise
...
# encoder_model = layers.Conv2D(num_of_filters, (2, 2), dilation_rate=1, activation='elu', padding='same', name='enc_conv2_1')(encoder_model)
# encoder_model = layers.Conv2D(num_of_filters, (3, 3), dilation_rate=1, activation='elu', padding='same', name='enc_conv2_2')(encoder_model)
...
attack_list = [jpeg_attaked, jpeg_attaked, jpeg_attaked, jpeg_attaked]
...
# decoder_model = layers.Conv2D(num_of_filters, (2, 2), dilation_rate=1, activation='elu', padding='same', name='dec_con2_1')(
#     decoder_model)
# decoder_model =layers.Conv2D(num_of_filters, (3, 3), dilation_rate=1, activation='elu', padding='same', name='dec_con2_2')(
#     decoder_model)

# decoder_model =conv2d_layer(num_of_filters * 2, (1, 1), dilation_rate=1, activation='elu', padding='same',
#                                   name='dec_output_depth2space_4')(decoder_model)
# decoder_model = conv2d_layer(num_of_filters, (1, 1), dilation_rate=1, activation='elu', padding='same',
...
model.compile(loss={'enc_output_depth2space':ssimmse_loss , 'dec_output_depth2space': 'binary_crossentropy'},#'mean_squared_error'},
              ...
              optimizer=optimizers.SGD(learning_rate=lr, momentum=0.98))


# %% Training 
exp_id = 'new_model_old_scheme'
...
for e in range(epochs):
    ...
    for step in tqdm(range(steps)):
        ...
        model_output = model([I, W], training=False)
