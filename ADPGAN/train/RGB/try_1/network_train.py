...
from RGB.try_1 import loss_functions_1
...
from RGB.try_1.discriminator import Discriminator
...
def UniformNoise(x, val):
    noise = tf.random.uniform(shape=[64, 4, 4, 192], minval=-val, maxval=val, dtype=tf.float32, seed=None)
    return x + noise
...
class ARWgan_RedMark():
    def __init__(self, batch_size, epochs):
        ...
        self.num_of_filters = (self.block_size ** 2)* 3
        self.use_circular = True
        self.input_img = layers.Input(shape=(self.img_rows, self.img_cols, 3), batch_size=self.batch_size, name='input_img')
    ...
    def build_encoder(self, x):
        ...
        rearranged_img = l1 = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': self.block_size},
                                            name='rearrange_img')(input_img)#(x,4,4,192)
    ...
    def build_attack(self, x):
        ...
        # q_mtx = q_mtx.reshape(64, 1)
        # n_channels = 192
        # x1 = np.arange(64)  # 原始索引 [0, 1, ..., 63]
        # x_new = np.linspace(0, 63, n_channels)  # 新索引，等分为 192 个点
        # q_mtx = interp1d(x1, q_mtx[:, 0], kind='cubic')
        # q_mtx = q_mtx(x_new)
        # q_mtx = np.round(q_mtx).astype(int)
        # q_mtx = np.maximum(q_mtx, 1)
        # q_mtx = q_mtx.reshape(192, 1)
        ...
        # q_mtx = np.reshape(q_mtx, (192, 1)) * 1.0
        ...
        noise_std = 5.0
        noise_attacked = layers.Activation(multiply_255)(x)
        ...
        #####################  Add GaussianNoise   ############################
        rounding_noise = layers.GaussianNoise(stddev=0.003, name='rounding_noise')(noise_attacked)
    ...
    def build_model(self):
        ...
        # Define loss
        ssim_win_size = 8
        loss_object = loss_functions_1.SSIM_MSE_LOSS(ssim_relative_loss=1.0, mse_relative_loss=0.0,
                                                   ssim_win_size=ssim_win_size)
        ssimmse_loss = loss_object.ssimmse_loss


        enc_output_weight =2.0
        dec_output_weight = 3.0
        discriminator_weight = 0.0001

        model.compile(loss={'enc_output_depth2space': 'mean_squared_error',
                      ...
                      optimizer=SGD(learning_rate=self.lr, momentum=0.98))
    ...
    def get_data(self):
        ...
        x_train_pascal = sio.loadmat('C:/code/PROJECT/ReDMark-2.15/images/pascal/RGB/image_resampled.mat')['patches']
    ...
    def train_epoch(self):
        exp_id = 'new_model_RGB_SGD_gausian'
...
if __name__ == '__main__':
    ARWgan_model = ARWgan_RedMark(batch_size=64, epochs=200)
    ARWgan_model.train_epoch()
