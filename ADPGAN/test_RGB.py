...
from RGB.try_1.decoder import Decoder
from RGB.try_1.encoder import Encoder
...
def buildModel(model_path, patch_rows=32, patch_cols=32, channels=1, block_size=8, use_circular=True):
    ...
    input_img = layers.Input(shape=(patch_rows, patch_cols, 3), name='input_img')
    ...
    # Rearrange input
    rearranged_img = l1 = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': block_size},
                                        name='rearrange_img')(input_img)


    dct_layer = layers.Conv2D(192, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                              name='dct1')
    dct_layer2 = layers.Conv2D(192, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                               name='dct2')
    idct_layer = layers.Conv2D(192, (1, 1), activation='linear', padding='same', use_bias=False, trainable=False,
                               name='idct')
    ...
    encoder_model = x = layers.Lambda(tf.nn.depth_to_space, arguments={'block_size': block_size},
                                      name='enc_output_depth2space')(encoder_model)

    # Attack (The attacks occure in test phase)

    # Watermark decoder
    input_attacked_img = layers.Input(shape=(patch_rows, patch_cols, 3), name='input_attacked_img')
    decoder_model = layers.Lambda(tf.nn.space_to_depth, arguments={'block_size': block_size},
                                  name='dec_input_space2depth')(input_attacked_img)
    # decoder_model = enhance_dense(num_of_filters=64, num_of_dense=32, name='pre_enhance')(decoder_model)
    ...
    # Set weights
    DCT_MTX = sio.loadmat('C:/code/PROJECT/ReDMark-2.15/transforms/RGB/DCT_coef.mat')['DCT_coef']
    dct_mtx = np.reshape(DCT_MTX, [1, 1, 192, 192])
    embedding_net.get_layer('dct1').set_weights(np.array([dct_mtx]))
    extractor_net.get_layer('dct2').set_weights(np.array([dct_mtx]))

    IDCT_MTX = sio.loadmat('C:/code/PROJECT/ReDMark-2.15/transforms/RGB/IDCT_coef.mat')['IDCT_coef']
    idct_mtx = np.reshape(IDCT_MTX, [1, 1, 192, 192])
...
test_folder = 'C:/code/PROJECT/ReDMark-2.15/images/{}x{}_rgb'.format(img_rows, img_cols)
test_imgs_files = [f for f in listdir(test_folder) if
                   isfile(join(test_folder, f)) and (f.endswith('.bmp') or f.endswith('.gif'))]


# Exp Info
# exp_id = 'new_model_rgb1'
exp_id = 'new_model_RGB_no_attack_SGD'
...
# Log folder
analysis_folder = 'C:/code/PROJECT/ReDMark-2.15/logs/{}/Analysis'.format(exp_id)
...
# Model Definition
model_path = 'C:/code/PROJECT/ReDMark-2.15/logs/{}/Weights/weights_140.h5'.format(exp_id)
...
def jpg(img, Q):
    global exp_id
    cv2.imwrite('C:/code/PROJECT/ReDMark-2.15/temporary_files/temp_{}.jpg'.format(exp_id), img, [cv2.IMWRITE_JPEG_QUALITY, int(Q)])
    # Iw_attacked = cv2.imread('temporary_files/temp_{}.jpg'.format(exp_id), cv2.IMREAD_GRAYSCALE)
    Iw_attacked = cv2.imread('C:/code/PROJECT/ReDMark-2.15/temporary_files/temp_{}.jpg'.format(exp_id))
    return Iw_attacked
...
attacks_list = [
    {'func': jpg, 'name': 'JPEG', 'params': np.array([70, 70, 70, 70, 70]), 'active': True},  # JPEG
    {'func': smoothing_mean, 'name': 'Average Filter', 'params': np.array([3, 5, 7, 9]), 'active': False},
    # Smoothing mean
    {'func': smoothing_gaussian, 'name': 'Gaussian Filter', 'params': np.arange(1, 3, 0.2), 'active': False},
    # Smoothing gaussian
    {'func': sharpening, 'name': 'Sharpening', 'params': np.arange(1, 50, 1), 'active': False},
    {'func': cropping_fillzero, 'name': 'Cropping ZeroFill', 'params': np.arange(0.01, 0.4, 0.05), 'active': False},
    # Cropping random patches and set to zero
    {'func': gaussin_noise, 'name': 'Gaussian Noise', 'params': np.arange(1, 30, 1), 'active': False},
    # Gaussian noise},
    {'func': salt_pepper_noise, 'name': 'Salt & Pepper Noise', 'params': np.arange(0.01, 0.2, 0.01), 'active': False},
    # Salt and pepper noise}
    {'func': median_filt, 'name': 'Median Filter', 'params': np.array([3, 5, 7, 9], dtype=np.int32), 'active': False},
    {'func': painting, 'name': 'Painting', 'params': np.array([1, 3, 6, 9, 12, 15, 16, 18], dtype=np.int32),
     'active': False},
    {'func': rotate, 'name': 'Rotation', 'params': np.arange(-10.0, 11.0, 1), 'active': False},
    {'func': gradient_attack, 'name': 'Edges', 'params': np.arange(0.5, 1.1, 0.1), 'active': False},
    {'func': resize_attack, 'name': 'Resizing', 'params': np.arange(0.25, 2.25, 0.25), 'active': False},
    {'func': grid_crop, 'name': 'Grid Crop', 'params': np.arange(0.01, 0.55, 0.05), 'active': False},
]

# %%
# Expriments
# alpha_values = np.array(range(1, 5 + 1, 1)) / (5.0)
alpha_values = np.array([1]) / (5.0)
...
for test_img in test_imgs_files:
    ...
    # Normalize image
    Im_normalized = (im.copy() - mean_normalize if Is_mean_normalized else 0) / std_normalize
    ...
    for alpha in alpha_values:
        ...
        for n in range(num_random_watermarks):
            Im_32x32_patchs = vf.RGB_partitioning(Im_normalized, p_size=patch_rows)
            ...
            # reconstruct Iw
            Iw = vf.RGB_tiling(Iw_batch, rec_size=img_rows)
            Iw *= std_normalize
            ...
            psnr = peak_signal_noise_ratio(im, Iw, data_range=255)
            tmp_psnr.append(psnr)
            # SSIM
            tmp_ssim.append(structural_similarity(im, Iw, win_size=9, data_range=255, channel_axis=2))

            # Save sample image
            if n == 0 and save_samples == True:
                # cv2.imwrite(os.path.join(sampled_embeded_folder, '{}_[{}].png'.format(test_img[:-4], alpha)), cv2.cvtColor(Iw, cv2.COLOR_RGB2BGR))
                cv2.imwrite(os.path.join(sampled_embeded_folder, '{}.bmp'.format(test_img[:-4])), cv2.cvtColor(Iw, cv2.COLOR_RGB2BGR))
...
#                     Iw_batch = embedding_net([Im_32x32_patchs, W_robust, alpha * np.ones_like(W_robust)],
#                                              training=False)
#
#                     # reconstruct Iw
#                     Iw = vf.RGB_tiling(Iw_batch, rec_size=img_rows)
...
#
#                     Iw_attacked_patchs = vf.RGB_partitioning(Iw_attacked, p_size=patch_rows)
#
#                     # Feed to extractor
