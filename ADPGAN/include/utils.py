...
def RGB_partitioning(unpartitioned, p_size):
    up_size = unpartitioned.shape[0]
    channels = unpartitioned.shape[2] if len(unpartitioned.shape) > 2 else 1
    parts = np.zeros((int((up_size/p_size)**2), p_size, p_size, channels))
    n = 0
    for u in range(int(up_size/p_size)):
        for v in range(int(up_size/p_size)):
            parts[n] = unpartitioned[p_size*u:p_size*u+p_size, p_size*v:p_size*v+p_size]
            n += 1
    return parts

def RGB_tiling(partitions, rec_size):
    p_size = partitions.shape[1]
    channels = partitions.shape[3]
    reconstructed_img = np.zeros((rec_size, rec_size, channels))
    n = 0
    for u in range(int(rec_size/p_size)):
        for v in range(int(rec_size/p_size)):
            reconstructed_img[p_size*u:p_size*u+p_size, p_size*v:p_size*v+p_size] = partitions[n]
            n += 1
    return reconstructed_img